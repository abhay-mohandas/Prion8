...
#define INSTR_CLRRB   0x64    // 01100100-CLRRB[8]      Clear all the Register Banks, excluding GPRs
#define INSTR_RFRB    0x68    // 01101000-RFRB[8]    	(R)ead (F)rom (R)egister (B)ank, values of [RB3:RB0] is read to [R3:R0]. Register Bank is pointed with the value in RA
#define INSTR_WFRB    0x6C    // 01101100-WFRB[8]	    (W)rite (t)o (R)egister (B)ank, values of [R3:R0] is written to [RB3:RB0]. Register Bank is pointed with the value in RA
#define INSTR_IO      0x70    // 01110000-IO[8]         Access I/O ports. I/O address in RA, value in IOR SPR. RA[7] = 0 for IN and 1 for OUT 
#define INSTR_CMP     0x74    // 01110100-CMP[8]		Compare, RB-RA and update flags
#define INSTR_LDH     0x78    // 01111000-LDH[8]		Load half word/2 bytes from address in MA0 to [RB:RA]
#define INSTR_STH     0x7C    // 01111100-STH[8]		Store half word/2 bytes to address in MA0 from [RB:RA]
